...
const defaultPrompts = {
  photoStudio: "Create a professional studio portrait. Combine the subjects from the provided images into one cohesive, photorealistic photo with studio lighting and a neutral, textured backdrop. Ensure subjects are scaled realistically based on their age.",
  profilePicturePro: "Generate a professional, high-quality corporate headshot of the subject. The lighting should be flattering and the background should be a simple, out-of-focus office or a neutral studio backdrop. The final image must be a head-and-shoulders composition.",
  vintageCamera: "Transform the photo to look like a vintage 35mm film photograph from the 1970s. Replicate the color saturation, grain, and soft focus characteristic of Kodachrome film.",
  backgroundScene: "Place the subjects from the source images into the described background scene. The final image must be photorealistic. Match the lighting, shadows, and colors of the subjects to the new background perfectly.",
  textToImage: "Create a photorealistic, cinematic, and highly detailed image based on the user's text prompt. The quality should be suitable for professional concept art or a promotional keyframe.",
  photoRestoration: "Restore the old, damaged photograph to its original condition. Repair all scratches, tears, and fading. Enhance clarity and correct color shifts, while preserving the photo's authentic vintage character.",
  productStudio: "Create a professional commercial product photograph. Isolate the product perfectly and place it on the described background. Use clean, studio lighting to highlight the product's features and quality. Generate realistic shadows and reflections.",
  styleTransfer: "Transfer the complete artistic style (color, texture, brushwork) from the 'Style Image' to the 'Content Image'. The final result should look as if the artist of the style image created a new piece of art depicting the content image's subject.",
  magicEraser: "Remove the object indicated by the white mask from the image. Fill in the area with a perfectly seamless, context-aware background that matches the original photo's lighting, texture, and perspective. The edit must be invisible.",
};
...
const PhotoStudio: React.FC<PhotoStudioProps> = ({ onBackToHome, mode }) => {
  ...
  const [prompt, setPrompt] = useState<string>(['photoStudio', 'profilePicturePro', 'vintageCamera', 'backgroundScene'].includes(mode) ? '' : defaultPrompts[mode]);
  ...
  const [loadingMessage, setLoadingMessage] = useState<string>("Generating your masterpiece...");
...
NEGATIVE PROMPT: ${negativePromptInstruction}
  ...
  const handleGenerate = async () => {
    ...
    setLoadingMessage(
        mode === 'textToImage' 
        ? "Crafting a high-quality image with Imagen..."
    ...
    try {
      ...
      const result = await generateImage(mode, base64Images, finalPrompt, aspectRatio);
  ...
  const handleGenerateVariations = async () => {
    ...
    try {
      ...
      const variationPromises = Array(4).fill(0).map(() => 
        generateImage(mode, base64Images, finalPrompt, aspectRatio)
      );
